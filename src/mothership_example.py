...
for i in range(dim_m_grid):
    for j in range(dim_m_grid):
        initial_m_state.add_location(i + dim_m_grid * j, 0.0, (i, j))


for n in range(dim_m_grid * dim_m_grid):
    for m in range(dim_m_grid * dim_m_grid):
        initial_m_state.add_path(n, m, 1.0)
